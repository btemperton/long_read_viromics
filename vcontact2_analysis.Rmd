...
library(knitr)
kable(L4_only_clusters %>% filter(q_value<0.05) %>%
...
gov_only_clusters<-contig_type_counts %>% filter(q_value<0.05) %>% filter(GOV/cluster_size==1)
gov_plus_L4_only_clusters<-contig_type_counts %>% filter(q_value<0.05) %>% filter((GOV + L4_count)/cluster_size==1)
...
per_cluster <- edge_map %>%
  filter(edge_classification %in% 
           c('GOV_GOV', 'GOV_L4_pilon', 'GOV_L4_short', 'GOV_L4_hybrid', 'GOV_RefSeq')) %>%
  ...
  mutate(p_value=wilcox.test(unlist(GOV_GOV), unlist(GOV_L4_pilon), alternative = 'greater', conf.int = TRUE)$p.value,
         conf.level=wilcox.test(unlist(GOV_GOV), unlist(GOV_L4_pilon), alternative = 'greater', conf.int = TRUE)$conf.int[1],
         estimate=wilcox.test(unlist(GOV_GOV), unlist(GOV_L4_pilon), alternative = 'greater', conf.int = TRUE)$estimate,
...
Is it to do with coverage? Are they lower abundance contigs?
